...
int decrease_count(int count)
{
    ...
    // spunem sa nu mai intre alt thread, sa astepte
    if (available_resources < count)
    {
        // verificam daca avem destul resurse, daca nu eliberam mutexul cu unlock
        // available_resources = 0;
        // printf("Got %d resources %d avaiable only\n", count, available_resources);
        pthread_mutex_unlock(&mtx);
        return -1;
    }
    else
    {
        // daca avem destule resurse le luam, dupa care le eliberam
...
void *myThread(void *v)
{
    ...
    while (decrease_count(*count))
        ; // cat timp nu se poate accesa resursa
    sleep(1);
...
int main()
{
    ...
    for (int i = 0; i < MAX_THREADS; i++)
    {
        ...
        *random = rand() % available_resources / 3 + 1;
        // *random =1;
        // printf("Random: %d\n", ((int)*random));
        if (pthread_create(&thrs[i], NULL, myThread, random))
        {
            // initializam thread-ul
            perror(NULL);
            return errno;
        }
    }
